...
EXPECTED_CARS = {"car_"+x for x in CAR_STEMS}
EXPECTED_VEHICLE_PARTS = EXPECTED_CARS | {"glass_"+x for x in CAR_STEMS} | {"trim_"+x for x in CAR_STEMS}
REC_SIZE = 84
...
def check_pak() -> pathlib.Path:
    path = check_binary("itajai_assets_v08.pak", 60000)
    data = path.read_bytes()
    if data[:8] != b"IDAPAK08": fail("asset PAK magic mismatch")
    if len(data) < 16: fail("asset PAK header truncated")
    version, count = struct.unpack_from("<II", data, 8)
    if version != 1: fail(f"unsupported asset PAK version {version}")
    if count < 16 or count > 32: fail(f"unexpected asset count {count}")
    table_end = 16 + count * REC_SIZE
    if table_end > len(data): fail("asset PAK record table truncated")
    names=set()
    for i in range(count):
        off=16+i*REC_SIZE;raw=data[off:off+32].split(b"\0",1)[0]
        try:name=raw.decode("utf-8")
        except UnicodeDecodeError: fail(f"asset #{i} has invalid UTF-8 name")
        names.add(name)
    missing=sorted(EXPECTED_CARS-names)
    if missing: fail("missing vehicle assets: "+", ".join(missing))
    if count>=32:
        missing_parts=sorted(EXPECTED_VEHICLE_PARTS-names)
        if missing_parts: fail("2.10 vehicle part contract incomplete: "+", ".join(missing_parts))
        print("OK PAK: 32 assets, 8 bodies + 8 glass + 8 trim vehicle parts present")
    else: print(f"OK PAK: {count} assets, all 8 production vehicles present")
    return path


def check_manifest(version: str, managed: tuple[pathlib.Path, ...]) -> None:
    path=DIST/"manifest.txt"
    if not path.is_file(): fail("manifest.txt missing")
    lines=[x.strip() for x in path.read_text(encoding="ascii").splitlines() if x.strip()];values={};files={}
    for line in lines:
        if line.startswith("file="):
            parts=line[5:].split("|")
            if len(parts)!=3: fail(f"bad manifest file line: {line}")
            name,size,digest=parts
            if not size.isdigit() or not re.fullmatch(r"[0-9a-f]{64}",digest): fail(f"bad manifest file metadata: {line}")
            if name in files: fail(f"duplicate manifest file: {name}")
            files[name]=(int(size),digest)
        elif "=" in line:
            k,v=line.split("=",1);values[k]=v
    if values.get("protocol")!="1": fail("manifest protocol is not 1")
    if values.get("version")!=version: fail("manifest VERSION mismatch")
    if not values.get("base_url","").endswith(f"/v{version}/"): fail("manifest base_url does not target current tag")
    expected_names={p.name for p in managed}
    if set(files)!=expected_names: fail(f"manifest managed set mismatch: {sorted(files)} != {sorted(expected_names)}")
