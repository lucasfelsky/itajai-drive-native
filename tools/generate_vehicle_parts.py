#!/usr/bin/env python3
"""Generate named vehicle sub-assets for the 2.10+ vehicle contract.
...
def glass_shell(v,i,sections):
    """Window shell only: left/right panes + windshield/rear glass, never roof."""
...
def glass_for(p):
    v=[];i=[];L=p['l'];W=p['w'];sections=[]
    # Sit a few centimetres outside the metal shell so the separate glass pass is
    # visible without z-fighting; roof remains body-coloured because no top quads.
    for zf,wf,base,top in p['roof']:
        sections.append((zf*L,wf*W*.505,base+.045,max(base+.10,top-.075)))
    glass_shell(v,i,sections);return v,i
...
def trim_for(p):
    ...
    if style=='box_hatch':box(v,i,0,.61,L*.509,W*.48,.12,.035)
    return v,i
